using System;
using System.Collections.Generic;
using System.Linq;
using System.Web;

namespace HDLERP.Reports.Entity
{
    public class GroupPerformanceDetail
    {
        public int ID { get; set; }
        public string CardNo { get; set; }
        public string CardNoK { get; set; }
        public string Name { get; set; }
        public string Name1 { get; set; }
        public string NameK { get; set; }
        public string FathersName { get; set; }
        public string MothersName { get; set; }
        public string Address1 { get; set; }
        public string Address2 { get; set; }
        public string Thana { get; set; }
        public string District { get; set; }
        public DateTime DateOfBirth { get; set; }
        public string Religion { get; set; }
        public string Education1 { get; set; }
        public string Education2 { get; set; }
        public string Education3 { get; set; }
        public string Education4 { get; set; }
        public string BloodGroup { get; set; }
        public string Designation { get; set; }
        public int DeptNo { get; set; }
        public string Department { get; set; }
        public DateTime DateOfJoining { get; set; }
        public DateTime OutOfFactory { get; set; }
        public decimal Basic { get; set; }
        public decimal HRent { get; set; }
        public decimal Medical { get; set; }
        public decimal Others { get; set; }
        public string Remarks { get; set; }
        public int NID { get; set; }
        public byte[] Photo { get; set; }
        public decimal GrossSalary { get; set; }
        public string ContNo { get; set; }
        public decimal Allowance { get; set; }
        public decimal AttBonus { get; set; }
        public string PoName { get; set; }
        public string PoCode { get; set; }
        public string CapName { get; set; }
        public string CapCode { get; set; }
        public DateTime Edate { get; set; }
        public string UserName { get; set; }
        public int GCode { get; set; }
        public string GName { get; set; }
        public string Machin { get; set; }
        public decimal Marks { get; set; }
        public string Evluation { get; set; }
        public int UCode { get; set; }
        public string UName { get; set; }
        public string IDNO { get; set; }
    }
}